...
Implementation Plan (file-spec-driven, Gemini-executed)

Phase 0 — Setup

Scaffold folders (already generated) + parent pom.xml + platform/common-* modules first.
File-specs for common-core/common-security/common-kafka/common-observability (everyone depends on these).

Phase 1 — Per-service, per-slice order (repeat for each of 7 services):

domain/ (aggregates, VOs, events, specs) — no dependencies, do first.
port/ (interfaces) — depends only on domain.
adapter/ + entity/ + repository/ — implements ports.
application/ (use cases) — wires domain + ports.
dto/, mapper/ — request/response shapes.
controller/ — thin, calls use cases.
config/, security/, validation/, exception/, event/, scheduler/ — cross-cutting, last.
Tests alongside each slice (unit with domain, integration after adapters exist).

Service build order (per earlier roadmap):
Merchant Service → Token Vault → Payment Orchestrator (stub Acquiring Adapter calls) → Acquiring Adapter (+ provider-simulator) → Webhook Service → Settlement Service → API Gateway last (needs all services routable).
...
Run tests, fix drift, then move to next slice.

Checkpoints:

After each service: run that service's integration tests standalone.
After Payment Orchestrator + Acquiring Adapter: run the concurrent-duplicate-idempotency test (proves no double-charge) — the resume-bullet-critical test.
After all 7: wire API Gateway, run one end-to-end payment.
Last: observability + load test.